...
def inputs_panel():
    ...
    st.sidebar.header("Purchase details")
    product_name = st.sidebar.text_input("Product name (e.g., BMW, Gold, Gucci bag)", value="BMW")
    product_price = st.sidebar.number_input("Product price", value=2500000.0, min_value=0.0, step=1000.0)
    down_payment = st.sidebar.number_input("Planned down payment", value=500000.0, min_value=0.0, step=10000.0)
    apr = st.sidebar.number_input("Loan APR (decimal, e.g., 0.10 for 10%)", value=0.10, min_value=0.0, step=0.005)
    term_years = st.sidebar.number_input("Loan term (years)", value=5, min_value=1, max_value=10, step=1)
